...
public class GreenDaoGenerator
{
    ...
    private static void initDownloader(Schema schema) throws Exception
    {
        ...
        downloader.addStringProperty("params").notNull(); //参数
        ...
        downloader.addStringProperty("checkKey").notNull();//文件流校验加密字符串
        downloader.addBooleanProperty("isBreakPoints").notNull();
        downloader.addIntProperty("errorCode").notNull();
        downloader.addStringProperty("errorMsg").notNull();
        ...
        downloader.addIntProperty("downloadType").notNull();//用于区分不同类型下载任务，在同一客户端存在多个下载中心时使用
    }
