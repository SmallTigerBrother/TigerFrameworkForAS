...
package com.mn.tiger.widget.viewflow.internal;
...
public class PLA_ListView extends PLA_AbsListView
{
	...
	public PLA_ListView(Context context, AttributeSet attrs)
	{
		...
		setDividerHeight(DisplayUtils.dip2px(context, 1));
	}
	 ...
	 * {@link android.widget.WrapperListAdapter}, depending on the ListView
	 * features currently in use. For instance, adding headers and/or footers
	 * will cause the adapter to be wrapped.
	 *
	 * @param adapter
	...
	{
		int count = getChildCount();
		return count > 0 ? getChildAt(0).getTop() - mDividerHeight : getHeight()
	...
	protected void measureScrapChild(View child, int position, int widthMeasureSpec)
	{
		...
		int childWidthSpec = ViewGroup.getChildMeasureSpec(widthMeasureSpec, mListPadding.left
	...
	protected int getScrapChildHeight(int position)
	{
		return obtainView(position, mIsScrap).getMeasuredHeight();
	}
	...
	private View fillSpecific(int position, int top)
	{
		...
		if (mDataChanged)
			Log.i("PLA_ListView", "fill specific: " + position + ":" + top);
	 ...
	 * {@link #setDivider(android.graphics.drawable.Drawable)}
	 *
	 * @param height
	 ...
	 * has stable IDs. ({@link android.widget.ListAdapter#hasStableIds()} ==
	 * {@code true})
	 *
	 * @return A new array which contains the id of each checked item in the
	 *         list.
