...
public class RoundedDrawable extends Drawable
{
	...
	public RoundedDrawable(Bitmap bitmap, float cornerRadius, int border, ColorStateList borderColor)
	{
		this(bitmap, cornerRadius, border, borderColor, false);
	}

	public RoundedDrawable(Bitmap bitmap, float cornerRadius, int border,
			ColorStateList borderColor, boolean oval)
	{
	...
	private void setMatrix()
	{
		...
		switch (mScaleType)
		{
			...
			case CENTER_INSIDE:
				...
				{
					scale = Math.min(mBounds.width() / (float) mBitmapWidth, mBounds.height()
							/ (float) mBitmapHeight);
				}
	...
	{
		return fromDrawable(drawable, radius, 0, ColorStateList.valueOf(DEFAULT_BORDER_COLOR),
				false);
	}

	public static Drawable fromDrawable(Drawable drawable, float radius, int border,
			ColorStateList borderColor, boolean isOval)
	{
		if (drawable != null)
		{
				...
				for (int i = 0; i < num; i++)
				{
					...
					else
					{
						drawableList[i] = new RoundedDrawable(drawableToBitmap(td.getDrawable(i)),
								radius, border, borderColor, isOval);
					}
				}
