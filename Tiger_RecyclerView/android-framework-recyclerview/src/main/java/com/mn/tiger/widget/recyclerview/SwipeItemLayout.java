...
public class SwipeItemLayout extends RelativeLayout
{
    ...
    // 拖动的弹簧距离
    private int mSpringDistance = 50;
    ...
    private BottomMode mBottomMode = BottomMode.PullOut;
    ...
    public void setSpringDistance(int springDistance)
    {
        this.mSpringDistance = springDistance;
    }
        ...
        if(null == mTopView)
        {
            if (getChildCount() != 2)
            {
                throw new IllegalStateException(
                        SwipeItemLayout.class.getSimpleName() + "必须有且只有两个子控件");
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        {
            mBottomView.measure(MeasureSpec.makeMeasureSpec(mBottomView.getMeasuredWidth(),
                    MeasureSpec.EXACTLY),
                    MeasureSpec.makeMeasureSpec(mTopView.getMeasuredHeight(), MeasureSpec.EXACTLY));
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b)
    {
        ...
        int bottomTop = topTop;
        int bottomBottom = topBottom;
        int bottomLeft;
        int bottomRight;
        ...
        if (mSwipeDirection == SwipeDirection.Left)
        {
            ...
            if (mBottomMode == BottomMode.LayDown)
            {
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback()
    {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy)
        {
            ...
            // 此时顶部视图水平方向偏移量的绝对值
            int topViewHorizontalOffset = Math.abs(
                    mTopLeft - (getPaddingLeft() + mTopLp.leftMargin));
